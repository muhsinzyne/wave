...
class RegisterController extends \App\Http\Controllers\Controller
{
    ...
    */
    ...
    protected function validator(array $data)
    {
        if (setting('auth.username_in_registration') && setting('auth.username_in_registration') == 'yes') {
            return Validator::make($data, [
                ...
                'mobile_no'  => 'required|string|min:12|max:12',
    ...
    public function getUniqueSubDomainFromStore($storeName)
    {
        $storeNameList    = explode(' ', strtolower($storeName));
        $storePrimaryName = '';
        if (count($storeNameList) > 0) {
            $storePrimaryName = $storeNameList[0];
        }

        $domainExisit = UserApps::where('sub_domain', '=', $storePrimaryName)->first();
        $counter      = 1;
        while (isset($domainExisit->id)) {
            $storePrimaryName = $storePrimaryName . $counter;
            $counter += 1;
            $domainExisit = UserApps::where('sub_domain', '=', $storePrimaryName)->first();
        }

        $domain = $storePrimaryName;

        // if (strlen($domain) < 4) {
        //     $domain = $domain . uniqid();
        // }
